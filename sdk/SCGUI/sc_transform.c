...
void sc_draw_trans(sc_pfb_t *dest, Transform_t *p)
{
    ...
    do
    {
        // sc_draw_Frame(dest, p->box.x, p->box.y, p->box.w,p->box.h, 1, C_RED, 255);
...
void sc_init_trans_label(sc_label_t *label, int cx, int cy, void *imge, Transform_t *p)
{
    ...
    font_dsc_t fdsc = sc_get_line_width(line_width_max, label, &indx, unicode, &g); // 获取label宽度
    size_t size = (fdsc.line_width * font->line_height) * g.bpp / 8;                        // 内存大小
    ...
    for (int k = 0; k<fdsc.line_cnt; k++)
    {
        if (font->get_glyph_dsc(font, &g, unicode[k], 0) == 0)
            continue;
        _label_to_mask(indx, 0, &g, unicode[k], font, mask, fdsc.line_width);
...
void sc_demo_transform(void)
{
    ...
    sc_init_Label(NULL, 0, 0, &label, gui->font, "SCGUI", 0);
