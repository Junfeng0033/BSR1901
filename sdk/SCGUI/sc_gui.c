...
void sc_draw_Fill(sc_pfb_t *dest, int xs, int ys, int w, int h, color_t color, uint16_t alpha)
{
    ...
    if (sc_pfb_intersection(dest, &rect, &intersection))
    {
        for (int y = intersection.ys; y < intersection.ye; y++)
        {
            ...
            {
                *dest_out = (alpha > 251) ? color : alphaBlend(color, *dest_out, alpha);
                dest_out++;
            }
        }
    }
        ...
        for (y = intersection.ys; y < intersection.ye; y++, dest_buf += dest_w)
        {
            ...
            if (y > ay && y < by) // 中间填充
            {
                for (x = intersection.xs; x < intersection.xe; x++, dest_out++)
                {
                    ...
                    else if (need_fill && x < fill_mask.xe)
                    {
                        *dest_out = (alpha > 251) ? fill : alphaBlend(fill, *dest_out, alpha);
                    }
                }
                continue;
            }
...
// 压缩图片绘制支持外部spi flash
void sc_draw_Image_zip(sc_pfb_t *dest, int xs, int ys, const SC_img_zip *zip, sc_dec_zip *dec, uint32_t spi_addr)
{
    sc_pfb_t tpfb; // 临时pfb
    sc_dirty_t intersection;
